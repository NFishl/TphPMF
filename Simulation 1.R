...
otu_real_data_T2D <- read.csv("/Users/hanxinyu/Desktop/otu_real_data_T2D-2.csv",  header = FALSE)
otu_real_data_control <- read.csv("/Users/hanxinyu/Desktop/otu_real_data_control-2.csv", header = FALSE)
D <- read.csv("/Users/hanxinyu/Desktop/D.csv", header = FALSE)
meta_data_T2D <- read.csv("/Users/hanxinyu/Desktop/meta_data_T2D-2.csv", header = FALSE)
meta_data_control <- read.csv("/Users/hanxinyu/Desktop/meta_data_control-2.csv", header = FALSE)
...
species_clusters <- cutree(hc, h = 5)
...
print(m)
print(n)
y_sim[y_sim == 0.00432137378264258] <- 0

write.csv(y_sim, "pmf1.csv")
...
imputed_value <- read.csv("/Users/hanxinyu/Desktop/mbimpute/pmf_simulated1_complete_data.csv",  header = FALSE)
colnames(imputed_value) <- imputed_value[1, ]
imputed_value <- imputed_value[-1,]
rownames(imputed_value) <- imputed_value[, 1]
imputed_value <- imputed_value[, -1]
imputed_value <- as.matrix(imputed_value)
imputed_value_vector <- as.vector(t(imputed_value))

impute_mat <- y_sim
for(i in 1:dim(idx_set)[1]){
  impute_mat[idx_set[i,1], idx_set[i,2]] = max(imputed_value_vector[i], log10(1.01))
}


y_sim <- impute_mat
write.csv(y_sim, "pmf_simulated1completedata.csv")
...
otable <- y_preserve
otable[] <- lapply(otable, as.numeric)
meta_tab <- meta_data_T2D
meta_tab[] <- lapply(meta_tab, as.numeric)
...
GapFilling(trait.info,hierarchy.info,num.samples=10, burn=2, gaps=1,
...
complete_data<-read.csv("/Users/hanxinyu/Desktop/mbimpute/pmf_simulated1completedata.csv", row.names = 1)

complete_data_matrix <- as.matrix(complete_data)
...
data <- read.table("/Users/hanxinyu/Desktop/bhpmf_sim11/mean_gap_filled.txt", header = TRUE, sep = "\t")
bhmpf_pre <- as.matrix(data)
bhmpf_pre_data<-t(bhmpf_pre)
y_preserve[] <- lapply(y_preserve, as.numeric)
sqrt(sum((sim_tab_zi - y_preserve)^2))
sim_tab_zi_mbImpute <- sim_tab_zi

write.csv(sim_tab_zi, "simulated_zi_matrix_mbImpute1.csv")
